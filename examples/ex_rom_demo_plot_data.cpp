#include <MOE/MOE.hpp> // road map to other files including in program
...
int main(int argc, char* argv[]) {
    ...
    // std::vector<double> filtered_acc(4,0.0);
    Eigen::VectorXd filtered_acc(4);
    // make options
    Options options("ex_rom_demo_plot_data.exe", "Demo showing range of motion of MOE");
    options.add_options()
        ...
        ("j,joint", "Joint to plot",value<int>());
    ...
    int active_joint = result["joint"].as<int>();
    ...
    while (!stop) {
        ...
        for (size_t i = 0; i < moe->n_j; ++i) {
            acc[i] = (moe->get_joint_velocities()[i] - last_vel[i])/(total_clock.get_elapsed_time().as_seconds() - last_time);
            last_vel[i] = moe->get_joint_velocities()[i];
            filtered_acc[i] = acc_filter[i].update(acc[i]);
        }


        last_time = total_clock.get_elapsed_time().as_seconds();
        plot_helper.add_data("G", grav_torques[active_joint]);
        plot_helper.add_data("V", moe->moe_dynamic_model.get_V()[active_joint]);
        plot_helper.add_data("Friction", moe->moe_dynamic_model.get_Friction()[active_joint]);
        plot_helper.add_data("Inertia",(moe->moe_dynamic_model.get_M()*filtered_acc)[active_joint]);
        plot_helper.add_data("Tau", command_torques[active_joint]);
        plot_helper.add_data("Calc Tau", grav_torques[active_joint] + moe->moe_dynamic_model.get_V()[active_joint] + moe->moe_dynamic_model.get_Friction()[active_joint] + (moe->moe_dynamic_model.get_M()*filtered_acc)[active_joint]);
