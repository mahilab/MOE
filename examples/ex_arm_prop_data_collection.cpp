...
while (!stop) {
    ...
    if (current_state == to_neutral) {
        ...
        for(std::size_t i = 0; i < moe->n_j; ++i) {
            if (i == active_joint) {
                ref[i] = neutral_point[i] + 10.0*DEG2RAD*sin(2*PI*(c_const[active_joint]/2.0*ref_traj_clock.get_elapsed_time().as_seconds()*ref_traj_clock.get_elapsed_time().as_seconds() + min_freq[active_joint]*ref_traj_clock.get_elapsed_time().as_seconds()));
            }
            else {
